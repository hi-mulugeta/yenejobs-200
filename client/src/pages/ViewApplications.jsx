...
const ViewApplications = () => {
  ...
  //Function to update Job applicatio status

  const chaneJobApplicationStatus = async (id, status) => {
    try {
      //
      const { data } = await axios.post(
        backendUrl + "/api/company/job-status",
        {
          id,
          status,
        },
                          ...
                          <div className="relative inline-block text-left group">
                            ...
                            <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150">
                              <div className="py-1">
                                <button
                                  onClick={() =>
                                    chaneJobApplicationStatus(
                                      applicant._id,
                                      "Accepted"
                                    )
                                  }
                                  className="block w-full text-left px-4 py-2 text-sm text-green-600 hover:bg-gray-100 transition-colors duration-150"
                                >
                                  <div className="flex items-center">
                                    <svg
